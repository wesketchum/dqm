...
ERS_DECLARE_ISSUE(triggertransform, CannotOpenFile, "Cannot open file : " << error, ((std::string)error))

ERS_DECLARE_ISSUE(triggertransform, ErrorReadingFile, "Error reading : " << error, ((std::string)error))

ERS_DECLARE_ISSUE(triggertransform, CannotOpenFolder, "Cannot open folder : " << error, ((std::string)error))

ERS_DECLARE_ISSUE(triggertransform, IncorrectParameters, "Incorrect parameters : " << fatal, ((std::string)fatal))

ERS_DECLARE_ISSUE(triggertransform, CannotProduce, "Cannot produce to kafka " << error, ((std::string)error))
} // namespace dunedaq
...
void
readDataset(std::string path_dataset, void* buff)
{
  ...
  if (path_dataset.find(tr_header) == std::string::npos) {
    ...
    if (frag.get_fragment_type() == dunedaq::daqdataformats::FragmentType::kTPCData) {
      ...
      for (size_t i = 0; i < raw_data_packets; i += interval_of_capture) {
        message_to_kafka = std::to_string(apa_count) + ";" + std::to_string(fragments_count) + ";" +
                           std::to_string(interval_of_capture) + ";" + std::to_string(frag.get_run_number()) + ";" +
                           std::to_string(frag.get_trigger_number()) + ";" +
                           std::to_string(frag.get_element_id().region_id) + ";" +
        ...
        try {
          ...
          RdKafka::ErrorCode err = m_producer->produce(topic,
                                                       ...
                                                       const_cast<char*>(message_to_kafka.c_str()),
                                                       message_to_kafka.size(),
                                                       nullptr,
                                                       0,
                                                       0,
                                                       nullptr,
          ...
          if (err != RdKafka::ERR_NO_ERROR) {
            ers::error(
              dunedaq::triggertransform::CannotProduce(ERS_HERE, "% Failed to produce " + RdKafka::err2str(err)));
          } else {
          ...
          // std::cout << s << std::endl;
          ers::error(
            dunedaq::triggertransform::CannotProduce(ERS_HERE, "Error [" + s + "] message(s) were not delivered"));
        }
      }
...
void
exploreSubGroup(HighFive::Group parent_group, std::string relative_path, std::vector<std::string>& path_list)
{
  int link_count = 0;
  bool is_link = false;
...
traverseFile(HighFive::File input_file, int num_trs)
{
  ...
  for (auto& dataset_path : path_list) {
    if (dataset_path.find("Fragment") == std::string::npos &&
        prev_ds_name.find("TriggerRecordHeader") != std::string::npos && i >= num_trs) {
      break;
    }
...
main(int argc, char** argv)
{
  ...
  bpo::options_description desc{
    "example: --rcemap /config/protoDUNETPCChannelMap_RCE_v4.txt --felixmap "
    "/config/protoDUNETPCChannelMap_FELIX_v4.txt --broker 127.0.0.1:9092 --Source defaultSource --topic "
        ...
        try {
          if (file.find("hdf5") != std::string::npos) {
            ...
            if (!file_parsed) {
              ...
              //  HighFive::File file("/eos/home-y/yadonon/swtest_run000002_0000_glehmann_20211001T115720.hdf5",
              //  HighFive::File::ReadOnly); std::vector<std::string> data_path = traverseFile(file, num_trs);
