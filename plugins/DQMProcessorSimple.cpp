...
void DQMProcessor::do_start_readout()
{
...
void DQMProcessor::do_drain_dataflow_readout()
{
  if (m_timesync_receiver)
...
void DQMProcessor::do_drain_dataflow_df()
{
...
DQMProcessor::do_drain_dataflow(const data_t&)
{
  ...
  if (m_mode == "readout")
    do_drain_dataflow_readout();
  else if (m_mode == "df")
    do_drain_dataflow_df();
}


bool
DQMProcessor::send_request_readout()
{
...
DQMProcessor::get_record_readout()
{
...
std::unique_ptr<daqdataformats::TriggerRecord>
DQMProcessor::get_record_df()
{
  std::unique_ptr<daqdataformats::TriggerRecord> element{ nullptr };
  if(dftrs.get_num_elements() == 0) {
...
void
DQMProcessor::get_data()
{

  bool request_sent=false;
  if( m_mode=="readout" )
    request_sent = send_request_readout();
  else if (m_mode=="df" )
    request_sent = send_request_df();
  ...
  std::unique_ptr<daqdataformats::TriggerRecord> element{ nullptr };
  if (m_mode == "readout")
    element.swap(get_record_readout());
  else if (m_mode == "df")
    element.swap(get_record_df());
